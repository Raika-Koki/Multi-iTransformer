...
while True:
    ...
    # WandBの初期化
    wandb.init(
        project=f"{stock_code}-stock-price-prediction-by-Transformer",
    ...
    for comp in components:
        ...
        num_variates = len(tickers)
        model, optimizer, scheduler = create_model(num_variates, params, depth, dim, predict_period_num)
        models[comp] = model
        ...
        # 学習済みモデルの保存
        torch.save(models[comp].state_dict(), f'{stock_code.lower()}_stock_price_{comp}_model_Transformer.pth')
