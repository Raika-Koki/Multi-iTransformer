...
while True:
    ...
    # WandBの初期化
    wandb.init(
        project=f"{stock_code}-stock-price-prediction-notMSTL-by-iTransformer",
    ...
    print(f"最適なハイパーパラメータが {file_name} に保存されました")
    ...
    # トレーニングループ
    model, optimizer, scheduler = create_model(best_params, len(tickers), predict_period_num, depth, dim)
    ...
    with torch.no_grad():
        ...
        predicted_stock_price = predicted[1][0, :, 0].cpu().numpy() * std_list.iloc[0] + mean_list.iloc[0]
    ...
    plt.title(f'{stock_code} Stock Price Prediction by iTransformer(No MSTL)', fontsize=18) #check

    # 保存するファイル名を一致させる
    plt.savefig(f'{stock_code.lower()}_stock_price_prediction_by_iTransformer(nomstl).png') # 修正
    # WandBにログを送信
    wandb.log({
        f"{stock_code} Stock Price Prediction by iTransformer(no MSTL)": wandb.Image(f'{stock_code.lower()}_stock_price_prediction_by_iTransformer(nomstl).png') # 修正
    })
