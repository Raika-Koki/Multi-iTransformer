...
# Transformer関連のimport (適切にパスを修正してください)
...
# モデル生成用
def create_model(params, pred_length, depth, dim):
    model = TransformerModel(
        num_variates=1,
...
file_name = f"best_hyperparameters_{stock_code}_Transformer(single).json"
...
while True:
    ...
    wandb.init(
        project=f"{stock_code}-stock-price-prediction-by-Transformer(single)", #check
    ...
    if all(comp in models for comp in data_dict):
        with torch.no_grad():
            ...
            predicted_trend_stock_price = predicted_trend[0, :, 0].cpu().numpy().flatten() * std_lists['trend'].iloc[0] + mean_lists['trend'].iloc[0]
            predicted_seasonal_0_stock_price = predicted_seasonal_0[0, :, 0].cpu().numpy().flatten() * std_lists['seasonal_0'].iloc[0] + mean_lists['seasonal_0'].iloc[0]
            predicted_seasonal_1_stock_price = predicted_seasonal_1[0, :, 0].cpu().numpy().flatten() * std_lists['seasonal_1'].iloc[0] + mean_lists['seasonal_1'].iloc[0]
            predicted_seasonal_2_stock_price = predicted_seasonal_2[0, :, 0].cpu().numpy().flatten() * std_lists['seasonal_2'].iloc[0] + mean_lists['seasonal_2'].iloc[0]
            predicted_seasonal_3_stock_price = predicted_seasonal_3[0, :, 0].cpu().numpy().flatten() * std_lists['seasonal_3'].iloc[0] + mean_lists['seasonal_3'].iloc[0]
            predicted_resid_stock_price = predicted_resid[0, :, 0].cpu().numpy().flatten() * std_lists['resid'].iloc[0] + mean_lists['resid'].iloc[0]
    ...
    plt.title(f'{stock_code} Stock Price Prediction by Transformer(single)', fontsize=18) #check

    # 保存するファイル名を一致させる
    plt.savefig(f'{stock_code.lower()}_stock_price_prediction_by_Transformer(single).png') # 修正
    # WandBにログを送信
    wandb.log({
        f"{stock_code} Stock Price Prediction by Transformer": wandb.Image(f'{stock_code.lower()}_stock_price_prediction_by_iTransformer(single).png') # 修正
    })
