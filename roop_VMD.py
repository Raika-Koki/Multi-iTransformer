...
def decompose_stock_data(data):
    ...
    signal = data.values
    # VMDによる分解
    # u: 各モード(成分)が行方向に並んだ2次元配列 (K, 信号長)
    u, u_hat, omega = VMD(signal, alpha=alpha, tau=tau, K=K, DC=DC, init=init, tol=tol)
    # 各モードを pandas.Series に戻してリストで返す
    mode_series_list = []
    index = data.index
    for i in range(K):
        mode_series = pd.Series(u[i, :], index=index)
        mode_series_list.append(mode_series)
    return mode_series_list
...
while True:
    ...
    vmd_series = decompose_stock_data(data_Adj_close)
    vmd_series_open = decompose_stock_data(data_open)

    # 分解結果のプロット（Adj Close）
    fig, axes = plt.subplots(7, 1, figsize=(12, 12), sharex=True)
    axes[0].plot(data_Adj_close, label='Original', color='black')
    axes[0].legend(loc='upper left')
    axes[0].set_title('Original Series')

    axes[1].plot(vmd_series[0], label='Mode 1', color='blue')
    axes[1].legend(loc='upper left')
    axes[1].set_title('Mode 1')

    axes[2].plot(vmd_series[1], label='Mode 2', color='green')
    axes[2].legend(loc='upper left')
    axes[2].set_title('Mode 2')

    axes[3].plot(vmd_series[2], label='Mode 3', color='orange')
    axes[3].legend(loc='upper left')
    axes[3].set_title('Mode 3')

    axes[4].plot(vmd_series[3], label='Mode 4', color='red')
    axes[4].legend(loc='upper left')
    axes[4].set_title('Mode 4')

    axes[5].plot(vmd_series[4], label='Mode 5', color='purple')
    axes[5].legend(loc='upper left')
    axes[5].set_title('Mode 5')

    axes[6].plot(vmd_series[5], label='Mode 6', color='grey')
    axes[6].legend(loc='upper left')
    axes[6].set_title('Mode 6')

    plt.suptitle(f"{stock_code} Adj Close VMD from {start_date} to {end_date}", fontsize=20)
    plt.tight_layout()
    plt.savefig("vmd_adj_close.png")
    plt.show()

    # 分解結果のプロット（Open）
    fig, axes = plt.subplots(7, 1, figsize=(12, 12), sharex=True)
    axes[0].plot(data_open, label='Original Open', color='black')
    axes[0].legend(loc='upper left')
    axes[0].set_title('Original Open')

    axes[1].plot(vmd_series_open[0], label='Mode 1', color='blue')
    axes[1].legend(loc='upper left')
    axes[1].set_title('Mode 1')

    axes[2].plot(vmd_series_open[1], label='Mode 2', color='green')
    axes[2].legend(loc='upper left')
    axes[2].set_title('Mode 2')

    axes[3].plot(vmd_series_open[2], label='Mode 3', color='orange')
    axes[3].legend(loc='upper left')
    axes[3].set_title('Mode 3')

    axes[4].plot(vmd_series_open[3], label='Mode 4', color='red')
    axes[4].legend(loc='upper left')
    axes[4].set_title('Mode 4')

    axes[5].plot(vmd_series_open[4], label='Mode 5', color='purple')
    axes[5].legend(loc='upper left')
    axes[5].set_title('Mode 5')

    axes[6].plot(vmd_series_open[5], label='Mode 6', color='grey')
    axes[6].legend(loc='upper left')
    axes[6].set_title('Mode 6')

    plt.suptitle(f"{stock_code} Open VMD from {start_date} to {end_date}", fontsize=20)
    plt.tight_layout()
    plt.savefig("vmd_open.png")
    plt.show()
    ...
    # トレンド、季節性、残差の成分を抽出
    stock_trend = vmd_series[0].tz_localize(None)
    stock_seasonal_0 = vmd_series[1].tz_localize(None)
    stock_seasonal_1 = vmd_series[2].tz_localize(None)
    stock_seasonal_2 = vmd_series[3].tz_localize(None)
    stock_seasonal_3 = vmd_series[4].tz_localize(None)
    stock_resid = vmd_series[5].tz_localize(None)

    stock_trend_open = vmd_series_open[0].tz_localize(None)
    stock_seasonal_0_open = vmd_series_open[1].tz_localize(None)
    stock_seasonal_1_open = vmd_series_open[2].tz_localize(None)
    stock_seasonal_2_open = vmd_series_open[3].tz_localize(None)
    stock_seasonal_3_open = vmd_series_open[4].tz_localize(None)
    stock_resid_open = vmd_series_open[5].tz_localize(None)
    ...
    # WandBの初期化
    wandb.init(
        project=f"{stock_code}-stock-price-prediction-by-iTransformer_VMD", # check
    ...
    plt.title(f'{stock_code} Stock Price Prediction by iTransformer_VMD', fontsize=18) #check

    # 保存するファイル名を一致させる
    plt.savefig(f'{stock_code.lower()}_stock_price_prediction_by_iTransformer_vmd.png') # 修正
    # WandBにログを送信
    wandb.log({
        f"{stock_code} Stock Price Prediction by iTransformer VMD": wandb.Image(f'{stock_code.lower()}_stock_price_prediction_by_iTransformer_vmd.png'),
        f"{stock_code} VMD Adj Close": wandb.Image("vmd_adj_close.png"),
        f"{stock_code} VMD Open": wandb.Image("vmd_open.png")
    })
